...
export default function SignUpStep2(props) {
    ...
    const funds = _.map(props.funds, (fund) => (<option key={fund.id} value={fund.id}>{ fund.name }</option>));

    if (props.fundId) {
        const fund = _.find(props.funds, ['id', props.fundId]);
        locations = _.map(fund.locations, (location) => (<option key={location.id} value={location.id}>{ location.name }</option>));
    }

    const trades = _.map(props.trades, (trade) => (<option key={trade.id} value={trade.id}>{ trade.name }</option>));

    const states = _.map(props.states, (name, abv) => (<option key={abv} value={abv}>{ name }</option>));

    const entityTypes = _.map(props.entityTypes, (entity) => (<option key={entity.id} value={entity.id}>{ entity.name }</option>));

    const title = <div style={styles.titleBar}><div style={styles.title}>Account Details</div><div style={styles.steps}>step 2 of 4</div></div>;

    const content = (<form onSubmit={props.nextAction}>
